---
layout: default
---

<div class="wrapper">
    <h1 class="page-title" data-i18n="nav_papers">Papers</h1>

    <div class="pub-section">
        <h2 class="pub-category-title" data-i18n="pub_main_author">Main Author</h2>

        <div class="pub-item">
            <div class="pub-text">
                <span class="author-highlight">García O’Diana, A.</span>, Quispe-Callo, G., & Vallejos Saldarriaga, J. (2025). 
                Validation of the Peace or Balance in Life Questionnaire (PBLQ-20): SEM and Network Analysis in Peruvian adolescents and young adults (Under Review). 
                <span class="journal-name">BMC Psychology</span>.
            </div>
            <div class="pub-badges">
                <a href="https://osf.io/y59br/" target="_blank" rel="noopener noreferrer" class="badge-link">OSF / Preprint</a>
            </div>
        </div>

        <div class="pub-item">
            <div class="pub-text">
                <span class="author-highlight">García O’Diana, A. A.</span>, Quispe-Callo, G. R., & Chicata, M. C. (2025). 
                Impact of Dengue Fear and Worry on Mental Health: A Psychometric Structural and Network Study (Under Review).
                <span class="journal-name">Journal of Psychiatric Research</span>.
            </div>
            <div class="pub-badges">
                <a href="https://osf.io/u83rv" target="_blank" rel="noopener noreferrer" class="badge-link">OSF / Preprint</a>
            </div>
        </div>

        <div class="pub-item">
            <div class="pub-text">
                <span class="author-highlight">García O’Diana, A. A.</span>, & Quispe-Callo, G. R. (2025). 
                Cross-Cultural Validation of the AAQ-II in five countries: A SEM and Psychometric Network Analysis approaches (Under Review). 
                <span class="journal-name">Europe’s Journal of Psychology</span>.
            </div>
            <div class="pub-badges">
                <a href="https://osf.io/8mpag" target="_blank" rel="noopener noreferrer" class="badge-link">OSF / Preprint</a>
            </div>
        </div>

        <div class="pub-item">
            <div class="pub-text">
                <span class="author-highlight">García O’Diana, A. A.</span> (2025). 
                Suicide Knowledge Test: Development and application of Psychometric Networks (Under Review). 
                <span class="journal-name">Death Studies</span>.
            </div>
            <div class="pub-badges">
                <a href="https://osf.io/92wzn" target="_blank" rel="noopener noreferrer" class="badge-link">OSF / Preprint</a>
            </div>
        </div>

        <div class="pub-item">
            <div class="pub-text">
                <span class="author-highlight">García O'Diana, A.</span>, Quispe Callo, G., Zumaran Baldeón, Á., Mamani, J. J., & Casa Chicata, M. (2024). 
                PsiNet LAB: Transformando la psicología con ciencia avanzada para una comprensión más profunda del comportamiento humano. 
                <span class="journal-name">Zenodo</span>.
            </div>
            <div class="pub-badges">
                <a href="https://doi.org/10.5281/zenodo.14042973" target="_blank" rel="noopener noreferrer" class="badge-link">DOI: 10.5281</a>
            </div>
        </div>

        <div class="pub-item">
            <div class="pub-text">
                <span class="author-highlight">García O'Diana, A.</span> (2024). 
                Comparative Analysis of Estimation Techniques in Network Analysis Applied to Research in University Teaching. 
                <span class="journal-name">SSRN</span>.
            </div>
            <div class="pub-badges">
                <a href="https://ssrn.com/abstract=4946225" target="_blank" rel="noopener noreferrer" class="badge-link">SSRN Link</a>
            </div>
        </div>

        <div class="pub-item">
            <div class="pub-text">
                <span class="author-highlight">García O'Diana, A.</span> (2021). 
                Peace or Balance, Agressive Behavior, Online Disinhibition, Moral Disengagement and Impulse Control in Peruvian University Students. 
                <span class="journal-name">SSRN</span>.
            </div>
            <div class="pub-badges">
                <a href="https://ssrn.com/abstract=4346941" target="_blank" rel="noopener noreferrer" class="badge-link">SSRN Link</a>
            </div>
        </div>
    </div>

    <div class="pub-section">
        <h2 class="pub-category-title" data-i18n="pub_co_author">Co-Author</h2>

        <div class="pub-item">
            <div class="pub-text">
                Ramos-Vera, C., Calle, D., <span class="author-highlight">O’Diana, A. G.</span>, Vallejos-Saldarriaga, J., Ramírez, Y. C., & Cubas, G. C. (2024). 
                Cross-Cultural Examination of the Bifactor Structure and Network Invariance of Dark Triad Items Across Four Countries. 
                <span class="journal-name">Interpersona: An International Journal on Personal Relationships</span>, 18(2), 265-286.
            </div>
            <div class="pub-badges">
                <a href="https://doi.org/10.5964/ijpr.10911" target="_blank" rel="noopener noreferrer" class="badge-link">DOI</a>
            </div>
        </div>

        <div class="pub-item">
            <div class="pub-text">
                Ramos-Vera, C., <span class="author-highlight">García O’Diana, A.</span>, Basauri-Delgado, M., Calizaya-Milla, Y. E., & Saintila, J. (2024). 
                Network analysis of anxiety and depressive symptoms during the COVID-19 pandemic in older adults in the United Kingdom. 
                <span class="journal-name">Scientific Reports</span>, 14(1), 7741.
            </div>
            <div class="pub-badges">
                <a href="https://doi.org/10.1038/s41598-024-58256-8" target="_blank" rel="noopener noreferrer" class="badge-link">DOI</a>
            </div>
        </div>

        <div class="pub-item">
            <div class="pub-text">
                Ramos-Vera C, <span class="author-highlight">García O’Diana A</span>, Calle D, Basauri-Delgado M, Bonfá-Araujo B, Lima-Costa AR, Duradoni M, Nasir S, Calizaya-Milla YE, Saintila J. (2024). 
                A Network Analysis Approach to Understanding Centrality and Overlap of 21 Dark Triad Items in Adults of 10 Countries. 
                <span class="journal-name">Psychol Res Behav Manag</span>, 17:467-483.
            </div>
            <div class="pub-badges">
                <a href="https://doi.org/10.2147/PRBM.S435871" target="_blank" rel="noopener noreferrer" class="badge-link">DOI</a>
            </div>
        </div>

        <div class="pub-item">
            <div class="pub-text">
                Ramos-Vera, C., <span class="author-highlight">García O’Diana, A.</span>, Basauri, M. D., Calle, D. H., & Saintila, J. (2023). 
                Psychological impact of COVID-19: A cross-lagged network analysis from the English Longitudinal Study of Aging COVID-19 database. 
                <span class="journal-name">Frontiers in Psychiatry</span>, 14.
            </div>
            <div class="pub-badges">
                <a href="https://doi.org/10.3389/fpsyt.2023.1124257" target="_blank" rel="noopener noreferrer" class="badge-link">DOI</a>
            </div>
        </div>

        <div class="pub-item">
            <div class="pub-text">
                Ramos-Vera, C., <span class="author-highlight">O’Diana, A. G.</span>, Vallejos-Saldarriaga, J., & Saintila, J. (2023). 
                Validation of the Post-Traumatic Growth Inventory in Adolescents Who Lost a Family Member in the Past 4 years: Effects of Time After the Death Event and Gender. 
                <span class="journal-name">OMEGA – Journal of Death and Dying</span>.
            </div>
            <div class="pub-badges">
                <a href="https://doi.org/10.1177/00302228231166833" target="_blank" rel="noopener noreferrer" class="badge-link">DOI</a>
            </div>
        </div>

        <div class="pub-item">
            <div class="pub-text">
                Ramos-Vera, C., <span class="author-highlight">O’Diana, A. G.</span>, Villena, A. S., et al. (2023). 
                Dark and Light Triad: A cross-cultural comparison of network analysis in 5 countries. 
                <span class="journal-name">Personality and Individual Differences</span>, 215, 112377.
            </div>
            <div class="pub-badges">
                <a href="https://doi.org/10.1016/j.paid.2023.112377" target="_blank" rel="noopener noreferrer" class="badge-link">DOI</a>
            </div>
        </div>

        <div class="pub-item">
            <div class="pub-text">
                Ramos-Vera, C., Saintila, J., <span class="author-highlight">O’Diana, A. G.</span>, & Calizaya-Milla, Y. E. (2022). 
                Identifying latent comorbidity patterns in adults with perceived cognitive impairment: Network findings from the behavioral risk factor surveillance system. 
                <span class="journal-name">Frontiers in Public Health</span>, 10, 981944.
            </div>
            <div class="pub-badges">
                <a href="https://doi.org/10.3389/fpubh.2022.981944" target="_blank" rel="noopener noreferrer" class="badge-link">DOI</a>
            </div>
        </div>
    </div>
</div>
